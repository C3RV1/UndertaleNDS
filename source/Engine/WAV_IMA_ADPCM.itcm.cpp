...
u32 WAV::ima_adpcm_get_sample_position(u32 sampleIdx, u8 channelIdx,
...
void WAV::progress_ima_adpcm_mono(u16 samples) {
  ...
  for (int i = 0; i < samples; i++) {
    ...
    u32 samplePos;
    samplePos = ima_adpcm_get_sample_position(_sourceBufferPos + i, 0, highBits,
                                              readHeader);
    if (readHeader) {
      u16 new_sample;
      u8 index;
...
void WAV::progress_ima_adpcm_stereo(u16 samples) {
  ...
  for (int i = 0; i < samples * 2; i++) {
    ...
    bool highBits, readHeader;
    u32 samplePos;
    samplePos = ima_adpcm_get_sample_position(_sourceBufferPos + i / 2,
                                              channelIdx, highBits, readHeader);
    if (readHeader) {
      u16 new_sample;
      u8 index;
      samplePos %= kAudioBuffer * 2;
      ...
      if (channelIdx == 0) {
        ...
        rightBuffer[(_sampleBufferPos + i / 2) % kAudioBuffer] =
            (s16)new_sample;
      }
      continue;
    }
