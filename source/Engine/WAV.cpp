...
void WAV::load(const std::string &name) {
  ...
  fseek(f, 4, SEEK_CUR); // skip byte rate == self.sample_rate *
                         // self.bits_per_sample * self.num_channels // 8
  fread(&_blockAlign, 2, 1,
        f); // TODO: Needed to be taken into account when decoding PCM?
...
void WAV::copy_from_file_buffer(u16 copy_length_samples) {
  if (!_stereo) {
    ...
    case SoundFormat_ADPCM:
      ...
      break;
    default:
      Engine::throw_("WAV: Format " + std::to_string(_format) +
                     " not implemented");
    }
  } else {
    switch (_format) {
...
bool WAV::renew_file_buffer() {
  ...
  if (!_stereo) {
    ...
    default:
      Engine::throw_("WAV: Format (2) " + std::to_string(_format) +
                     " not implemented");
    }
  } else {
    switch (_format) {
