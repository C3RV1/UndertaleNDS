...
void Background::loadCBGF(FILE *f) {
  ...
  if ((colorCount > 249 && _color8bit) || (colorCount > 15 && !_color8bit)) {
    std::string buffer = "Error loading bg #r" + _path +
                         "#x: Color count does not match 8 bit flag (colors: " +
  ...
  *bg3Reg = (*bg3Reg & (~0xC000)) + sizeFlag;
  ...
  for (int mapX = 0; mapX < (_mapWidth + 31) / 32; mapX++) {
    ...
    for (int mapY = 0; mapY < (_mapHeight + 31) / 32; mapY++) {
      u8 *mapStart =
          (u8 *)mapRam + (mapY * ((_mapWidth + 31) / 32) + mapX) * 2048;
      dmaFillSafe(3, 0, mapStart, 0x800);
      for (int row = mapY * 32; row < _mapHeight && row < (mapY + 1) * 32;
           row++) {
...
int Background::loadBgExtendedSub(int forceSize) {
  ...
  return loadBgExtendedEngine(&REG_BG3CNT_SUB, BG_PALETTE_SUB,
                              BG_TILE_RAM_SUB(1), BG_MAP_RAM_SUB(0),
                              &REG_BG3PA_SUB, &REG_BG3PB_SUB, &REG_BG3PC_SUB,
                              &REG_BG3PD_SUB, forceSize);
}

int Background::loadBgExtendedEngine(vu16 *bg3Reg, u16 *paletteRam,
                                     u16 *tileRam, u16 *mapRam, vs16 *reg3A,
                                     vs16 *reg3B, vs16 *reg3C, vs16 *reg3D,
                                     int forceSize) {
  if (!_loaded)
    return 1;
...
int Background::loadBgRectMain(int x, int y, int w, int h) {
  return loadBgRectEngine(&REG_BG3CNT, BG_TILE_RAM(1), BG_MAP_RAM(0), x, y, w,
                          h);
}
  ...
  for (int row = y; row < y + h; row++) {
    for (int col = x; col < x + w; col++) {
      ...
      *mapRes = tileDst;
      ...
      int srcCol = col;
      auto *mapSrc =
          (u16 *)((u8 *)_map.get() + (srcRow * _mapWidth + srcCol) * 2);
      ...
      if (_color8bit) {
        ...
        for (int i = 0; i < 64; i++) {
          ...
          tileRes[i / 2] |= (src[i / 2] >> (4 * highBits) & 0xF)
                            << (8 * highBits);
        }
      }
    }
