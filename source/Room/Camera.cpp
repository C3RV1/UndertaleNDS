...
void Camera::updatePosition(bool roomChange) {
  ...
  if (roomChange) {
    ...
    for (int xTile = xTilePrev; xTile != xTilePost; xTile += incrementX) {
      globalRoom->_bg.loadBgRectMain(xTile + incrementX + 32, yTilePost - 1, 1,
                                     26);
      globalRoom->_bg.loadBgRectMain(xTile + incrementX - 1, yTilePost - 1, 1,
                                     26);
    }
    for (int yTile = yTilePrev; yTile != yTilePost; yTile += incrementY) {
      globalRoom->_bg.loadBgRectMain(xTilePost - 1, yTile + incrementY + 24, 34,
                                     1);
      globalRoom->_bg.loadBgRectMain(xTilePost - 1, yTile + incrementY - 1, 34,
                                     1);
    }
  }
