...
void Camera::updatePosition(bool roomChange, Player& player) {
  constexpr int offsetX = 0, offsetY = -20;
  int xTilePrev = (_prevX >> 8) / 8, yTilePrev = (_prevY >> 8) / 8;
  if (!_manual) {
    _pos->_wx = player._spr->_wx - ((256 / 2 - 9) << 8) + (offsetX << 8);
    _pos->_wy = player._spr->_wy - ((192 / 2 - 14) << 8) + (offsetY << 8);
  }
  u16 roomW, roomH;
  _room->_bg.getSize(roomW, roomH);
  if (roomW >= 256) {
  ...
  if (roomChange) {
    ...
    for (int xTile = xTilePrev; xTile != xTilePost; xTile += incrementX) {
      _room->_bg.loadBgRectMain(xTile + incrementX + 32, yTilePost - 1, 1,
                                     26);
      _room->_bg.loadBgRectMain(xTile + incrementX - 1, yTilePost - 1, 1,
                                     26);
    }
    for (int yTile = yTilePrev; yTile != yTilePost; yTile += incrementY) {
      _room->_bg.loadBgRectMain(xTilePost - 1, yTile + incrementY + 24, 34,
                                     1);
      _room->_bg.loadBgRectMain(xTilePost - 1, yTile + incrementY - 1, 34,
                                     1);
    }
  }
