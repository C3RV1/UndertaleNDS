...
class ObjCondition:
    ...
    def __init__(self, flag: int, cmp: int | str, cmp_value: int) -> None:
        self.flag: int = flag
...
class ObjConditionAndValue[T](ObjData[T]):
    ...
    def write(self, obj: Obj, wtr: binary.BinaryWriter,
        ...
        has_next_variation = isinstance(self.next, ObjConditionAndValue[T])
...
class RoomSprite(Obj):
    ...
    def write(self, wtr: binary.BinaryWriter) -> None:
        self.spr_id.write(self, wtr, wtr.write_int16)
    ...
    def from_json_obj(cls, json_obj: dict[str, Any]) -> RoomSprite:
        ...
        return cls(
            unpack_data(json_obj.get("id", -1), int_unpacker),
...
class RoomCollider(Obj):
    ...
    def write(self, wtr: binary.BinaryWriter) -> None:
        self.coll_id.write(self, wtr, wtr.write_int16)
        for v in self.rect:
            v.write(self, wtr, wtr.write_uint16)
...
def compile_rooms(force: bool = False) -> None:
    for root, _, files in os.walk("new_rooms"):
